...
fld.flag_p_bound    = 1;                    % [-]       flag, which boundary conditions for the hydrodynamic pressure are used: 
...
% Cavity fraction:
fld.flag_thet_bound    = 1;                 % [-]       flag, which boundary conditions for the hydrodynamic pressure are used: 
% 0) Dirichlet at West, North, South and East boundary
% 1) Dirichlet at West and East boundary, Neumann at North and South boundary
% 2) Dirichlet at West boundary, Neumann at North, South and East boundary
...
% Dynamic viscosity:
fld.mu_0        = 1e-2;                                                   % [Pas]     lubricant dynamic viscosity at ambient pressure
...
%% Solid settings:
sld.flag_elastic    = false;                                                % [-]   flag whether solid body is elastic on macroscopic scale (true) or rigid (false)
if sld.flag_elastic
...
opc.u_up_ini            = autorun.opc.u_up_ini(autorun.sim.it);                                    % [m/s] velocity of upper body in x1-direction, the disk
...
%% Geometry settings:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Study  E: Inclined surfaces with pocket - Fowell et al. et al., 2007 - Giacoponi et al. 2010:
% Input:
% Gap height:
geo.h_min           = 1.0e-6;       % [m]       minimum gap height
geo.h_max           = geo.h_min*(1 + 0.05);       % [m]       maximum gap height
geo.h_p             = 1*geo.h_min;       % [m]       dimple depth

% x1-direction:
geo.L_x1            = 1e-2;                             % [m]       domain lenght
geo.l_x1            = 3.0e-3;                           % [m]       dimple length
geo.l_p_x1          = 2.0e-3;                           % [m]       distance between dimple and inlet
geo.Nx1             = autorun.geo.Nx1(autorun.sim.it);                              % [-]       number of discretization cells
geo.x1              = linspace(0,geo.L_x1,geo.Nx1);     % [m]       coordinate of cell centers
geo.dx1             = geo.L_x1/(geo.Nx1 - 1);           % [m]       spacing

% x2-direction: pseudo 1D:
geo.Nx2             = 2^1+1;                            % [-]       number of discretization cells
geo.dx2             = geo.dx1;                          % [m]       spacing
geo.L_x2            = (geo.Nx2 - 1)*geo.dx2;            % [m]       domain width
geo.x2              = linspace(0,geo.L_x2,geo.Nx2);     % [m]       coordinate of cell centers

% Computation:
[x1_matr,~] = ndgrid(geo.x1,geo.x2);
...
alg.FBNS.alpha_p_nd     = 5e-1;         % [-]   relaxation factor for update of hydrodynmic pressure
alg.FBNS.alpha_thet     = 5e-1;         % [-]   relaxation factor for update of cavity fraction
